...
## 1.2.0 marketplace hardening

Plugin id is now `io.github.mikus2604.hotkey-hints`. The watcher is no longer
executed from the user-writable plugin checkout.

- `scripts/install.sh` copies `hotkey-watcher.py` to
  `/usr/local/libexec/omarchy-hotkey-hints/` as root-owned `0755` and writes a
  systemd unit whose `ExecStart` is that path plus `--uid <session>`.
- The watcher starts as root, opens `/dev/input`, then `seteuid` to `--uid`.
  User files and `omarchy-shell` IPC run as the session user.
- Default capture is the eight modifier keycodes. Non-modifier keys are
  inspected only when `watch.json` has `rememberUsage: true`.
- Usage counts are written by the watcher to
  `~/.local/state/omarchy/hotkey-hints/usage.json`. Overlay IPC no longer has
  `used` / `resetUsage`.
- QML `FileView` is watcher-only. Reads/writes go through
  `scripts/state-file.py` (`O_NOFOLLOW|O_NONBLOCK`, atomic rename).
- Keybindings dump goes through `scripts/print-keybindings.sh` (byte cap +
  deadline). No `StdioCollector`.
- `CLAUDE.md` removed from the installable tree (`docs/CONTRIBUTING.md`
  instead).

The Hyprland `bindr` investigation below is unchanged and still the reason a
kernel-side watcher exists.
...
If you still have the 1.1 `t480.hotkey-hints` plugin and its user-checkout
unit, disable that unit first so two watchers do not fight.

Requirements: `python-evdev` (system package). The helper is copied to a
root-owned path; `/dev/input` is opened as root, then the process `seteuid`s
to the session user.

## Operational notes / gotchas

- The live unit is generated by `scripts/install.sh`. Its `ExecStart` is the
  libexec copy plus `--uid`. Do not point systemd at the plugin checkout.
- Session env comes from `--uid` / `pwd.getpwuid`, not from globbing
  `/run/user/*/wayland-*`.
- `sudo killall python3` can match the watcher — use
  `systemctl restart omarchy-hotkey-hints-watcher`.
